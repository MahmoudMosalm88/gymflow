...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = await query<GuestPassRow>(
      `SELECT id, code, member_name, phone, expires_at, used_at, created_at
         FROM guest_passes
  ...
  try {
    ...
    const rows = await query<GuestPassRow>(
      `INSERT INTO guest_passes (
          id, organization_id, branch_id, code, member_name, phone, expires_at
       ) VALUES (
          $1, $2, $3, $4, $5, $6, $7
       )
       RETURNING id, code, member_name, phone, expires_at, used_at, created_at`,
      [uuidv4(), auth.organizationId, auth.branchId, code, memberName, body.phone || null, expiresAt.toISOString()]
    );
