...
async function writeStatus(organizationId, branchId, value) {
    await pool.query(`INSERT INTO settings (organization_id, branch_id, key, value)
     ...
     DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()`, [organizationId, branchId, JSON.stringify(value)]);
}
...
async function processTenantQueue(runtime) {
    if (!runtime.sock || !runtime.isReady)
        return;
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        const result = await client.query(`SELECT mq.id, mq.member_id, mq.type, mq.payload,
        ...
        ORDER BY
          ...
          mq.scheduled_at ASC
        LIMIT $3
        FOR UPDATE SKIP LOCKED`, [runtime.organizationId, runtime.branchId, workerBatchLimit]);
        for (const row of result.rows) {
            await client.query(`UPDATE message_queue
            SET status = 'processing', attempts = attempts + 1
          WHERE id = $1`, [row.id]);
            if (dryRun) {
                await client.query(`UPDATE message_queue SET status = 'sent', sent_at = NOW() WHERE id = $1`, [row.id]);
                console.log(`[${runtime.key}] [DRY RUN] marked sent queue=${row.id} type=${row.type}`);
                continue;
            }
            try {
                const payload = row.payload;
                const rawPhone = String(row.member_phone || (typeof payload?.phone === "string" ? payload.phone : ""));
...
async function scheduleRenewalRemindersForTenant(organizationId, branchId) {
    ...
    const result = await pool.query(`SELECT s.id AS subscription_id, s.member_id, s.end_date, m.name, m.phone
    ...
    for (const row of result.rows) {
        ...
        LIMIT 1`, [organizationId, branchId, row.member_id, String(row.subscription_id), String(daysLeft)]);
