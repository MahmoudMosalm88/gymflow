...
const postExpirySteps = [0, 3, 7, 14];
const defaultRenewalTemplateEn = "Hi {name}, your subscription will expire on {expiryDate} ({daysLeft} days left). Please renew to keep access active.";
const defaultRenewalTemplateAr = "مرحباً {name}، ينتهي اشتراكك بتاريخ {expiryDate} (متبقي {daysLeft} أيام). يرجى التجديد للحفاظ على العضوية.";
...
async function throttleSend() {
    if (minSendIntervalMs <= 0 && sendJitterMs <= 0)
        return;
    const jitter = sendJitterMs > 0 ? Math.floor(Math.random() * (sendJitterMs + 1)) : 0;
    await sleep(minSendIntervalMs + jitter);
}
function toTenantKey(organizationId, branchId) {
    return `${organizationId}:${branchId}`;
}
function sanitizePathSegment(value) {
    return value.replace(/[^a-zA-Z0-9_-]/g, "_");
}
function toAuthPath(organizationId, branchId) {
...
function normalizeSystemLanguage(value, fallback = "en") {
...
function getTemplateKey(type, lang) {
...
function isStaffInvitePayload(payload) {
    return payload?.kind === "staff_invite";
}
function isStaffInvitesOnlyMode(settings) {
...
function formatLocalizedDate(unixSeconds, lang) {
...
function getRuntime(organizationId, branchId) {
...
async function writeStatus(organizationId, branchId, value) {
    await pool.query(`INSERT INTO settings (organization_id, branch_id, key, value)
     VALUES ($1, $2, 'whatsapp_status', $3::jsonb)
     ON CONFLICT (organization_id, branch_id, key)
     DO UPDATE SET value = COALESCE(settings.value, '{}'::jsonb) || EXCLUDED.value, updated_at = NOW()`, [organizationId, branchId, JSON.stringify(value)]);
}
async function refreshCampaignStatus(client, campaignId) {
    if (!campaignId)
        return;
    await client.query(`WITH stats AS (
      ...
      UPDATE whatsapp_campaigns wc
         SET recipient_count = stats.recipient_count,
             ...
             END
        FROM stats
       WHERE wc.id = $1`, [campaignId]);
}
async function listTenantStatuses() {
    const res = await pool.query(`SELECT organization_id, branch_id, value
       FROM settings
...
async function listQueuedTenants() {
    const res = await pool.query(`SELECT DISTINCT organization_id, branch_id
       FROM message_queue
...
async function readTenantSettings(organizationId, branchId) {
    const rows = await pool.query(`SELECT key, value
       FROM settings
...
function isLifecycleFlagEnabled(settings, key) {
    if (!lifecycleAutomationsEnabled)
        return false;
...
async function disconnectRuntime(runtime, logout) {
...
async function ensureRuntimeSocket(runtime) {
    if (runtime.connecting || runtime.sock || runtime.desiredState === "disconnected")
        return;
    ...
    try {
        ...
        sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
            ...
            if (connection === "close") {
                ...
                console.warn(`[${runtime.key}] connection closed code=${code ?? "n/a"} loggedOut=${loggedOut} stopRequested=${stopRequested} reason=${reasonText}`);
...
async function processTenantQueue(runtime) {
    if (!dryRun && (!runtime.sock || !runtime.isReady))
        return;
    ...
    try {
        ...
        const result = await client.query(`SELECT mq.id, mq.member_id, mq.campaign_id, mq.type, mq.payload,
              mq.target_phone,
              mq.target_name,
        ...
        ORDER BY
          ...
          mq.scheduled_at ASC
        LIMIT $3
        FOR UPDATE OF mq SKIP LOCKED`, [runtime.organizationId, runtime.branchId, workerBatchLimit]);
        for (const row of result.rows) {
            await client.query(`UPDATE message_queue
            SET status = 'processing', attempts = attempts + 1, last_attempt_at = NOW()
          WHERE id = $1`, [row.id]);
            await refreshCampaignStatus(client, row.campaign_id);
            if (dryRun) {
                await client.query(`UPDATE message_queue SET status = 'sent', sent_at = NOW(), provider_message_id = $2 WHERE id = $1`, [row.id, `dry-run:${row.id}`]);
                await refreshCampaignStatus(client, row.campaign_id);
            ...
            try {
                ...
                SET status = 'failed',
                    ...
                    last_error = $2
              WHERE id = $1`, [row.id, "Suppressed by whatsapp_staff_invites_only safe mode"]);
                    await refreshCampaignStatus(client, row.campaign_id);
                    console.warn(`[${runtime.key}] suppressed queue=${row.id} type=${row.type} by safe mode`);
                    continue;
                }
                const rawPhone = String(row.member_phone ||
                    row.target_phone ||
                ...
                await throttleSend();
                await client.query(`UPDATE message_queue SET status = 'sent', sent_at = NOW(), provider_message_id = $2 WHERE id = $1`, [row.id, sentId ?? null]);
                await refreshCampaignStatus(client, row.campaign_id);
                console.log(`[${runtime.key}] sent queue=${row.id} type=${row.type} jid=${jid} waMessageId=${sentId ?? "n/a"}`);
            }
            catch (error) {
...
async function scheduleRenewalRemindersForTenant(organizationId, branchId) {
    ...
    if (!automationEnabled || isStaffInvitesOnlyMode(settings))
        return;
    ...
    const languageTemplate = typeof settings[languageTemplateKey] === "string"
        ? String(settings[languageTemplateKey]).trim()
        : "";
    const fallbackTemplate = typeof settings.whatsapp_template_renewal === "string"
        ? settings.whatsapp_template_renewal.trim()
        : "";
    const legacyFallback = systemLanguage === "en" ? fallbackTemplate : "";
    const renewalTemplate = languageTemplate ||
        legacyFallback ||
    ...
    const result = await pool.query(`SELECT s.id AS subscription_id, s.member_id, s.start_date, s.end_date, m.name, m.phone
      ...
      WHERE s.is_active = true
        ...
        AND s.start_date <= $3
        AND s.end_date > $3
        AND s.end_date <= $4`, [organizationId, branchId, nowSec, maxWindowSec]);
    for (const row of result.rows) {
        if (!row.phone)
            continue;
        ...
        if (exists.rows.length > 0)
            continue;
        const expiryDate = new Date(Number(row.end_date) * 1000).toLocaleDateString(systemLanguage === "ar" ? "ar-EG" : "en-US", {
            day: "2-digit",
            month: "short",
            year: "numeric",
        });
        ...
        await pool.query(`INSERT INTO message_queue (id, organization_id, branch_id, member_id, type, payload, status, scheduled_at)
...
async function schedulePostExpirySequencesForTenant(organizationId, branchId) {
    const settings = await readTenantSettings(organizationId, branchId);
    const automationEnabled = parseBooleanSetting(settings.whatsapp_automation_enabled, true);
    if (!automationEnabled || isStaffInvitesOnlyMode(settings))
        return;
    if (!isLifecycleFlagEnabled(settings, "whatsapp_post_expiry_enabled"))
        return;
    const systemLanguage = normalizeSystemLanguage(settings.system_language, "en");
    const rows = await pool.query(`WITH latest_expired AS (
    ...
    for (const row of rows.rows) {
        ...
        if (!postExpirySteps.includes(daysSinceExpiry))
            continue;
        const exists = await pool.query(`SELECT 1
         FROM message_queue
        WHERE organization_id = $1
        ...
        LIMIT 1`, [organizationId, branchId, row.member_id, String(row.subscription_id), String(daysSinceExpiry)]);
        if (exists.rows.length > 0)
            continue;
        const template = defaultPostExpiryTemplates[systemLanguage][daysSinceExpiry];
        ...
        await pool.query(`INSERT INTO message_queue (
...
async function scheduleOnboardingSequencesForTenant(organizationId, branchId) {
    const settings = await readTenantSettings(organizationId, branchId);
    const automationEnabled = parseBooleanSetting(settings.whatsapp_automation_enabled, true);
    if (!automationEnabled || isStaffInvitesOnlyMode(settings))
        return;
    if (!isLifecycleFlagEnabled(settings, "whatsapp_onboarding_enabled"))
        return;
    const systemLanguage = normalizeSystemLanguage(settings.system_language, "en");
    const rows = await pool.query(`WITH recent_members AS (
     ...
     SELECT rm.member_id,
       ...
       LEFT JOIN activity a ON a.member_id = rm.member_id`, [organizationId, branchId]);
    const nowSec = Math.floor(Date.now() / 1000);
    for (const row of rows.rows) {
        if (!row.phone)
            continue;
        ...
        if (firstVisit) {
          ...
          LIMIT 1`, [organizationId, branchId, row.member_id]);
            if (exists.rows.length === 0) {
...
async function scheduleWeeklyDigestForTenant(organizationId, branchId) {
    const settings = await readTenantSettings(organizationId, branchId);
    const automationEnabled = parseBooleanSetting(settings.whatsapp_automation_enabled, true);
    if (!automationEnabled || isStaffInvitesOnlyMode(settings))
        return;
    if (!isLifecycleFlagEnabled(settings, "whatsapp_weekly_digest_enabled"))
        return;
    const now = new Date();
    ...
    if (dayOfWeek !== 1 || hour < 8)
        return;
    const alreadyQueued = await pool.query(`SELECT 1
       FROM message_queue
      WHERE organization_id = $1
        AND branch_id = $2
        AND payload->>'sequence_kind' = 'weekly_digest'
        AND created_at >= NOW() - interval '7 day'
      LIMIT 1`, [organizationId, branchId]);
    if (alreadyQueued.rows.length > 0)
        return;
    const ownerRows = await pool.query(`SELECT o.id AS owner_id, o.name AS owner_name, o.phone AS owner_phone
       FROM owner_branch_access oba
       JOIN owners o ON o.id = oba.owner_id
      WHERE oba.branch_id = $1
        AND o.phone IS NOT NULL`, [branchId]);
    if (ownerRows.rows.length === 0)
        return;
    const [riskRows, retentionRows, roiRows, atRiskRows] = await Promise.all([
        pool.query(`SELECT COALESCE(SUM(price_paid), 0)::text AS total,
        ...
        WHERE organization_id = $1
          ...
          AND start_date <= $3
          AND end_date > $3
          AND end_date <= $4`, [organizationId, branchId, Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000) + 14 * 86400]),
        pool.query(`WITH active_start AS (
       ...
       SELECT
         ...
         (SELECT COUNT(*)::int FROM active_start s JOIN active_end e ON e.member_id = s.member_id) AS retained_members`, [organizationId, branchId, Math.floor(Date.now() / 1000) - 30 * 86400, Math.floor(Date.now() / 1000)]),
        pool.query(`WITH sent_messages AS (
         SELECT mq.member_id,
       ...
       SELECT COALESCE(SUM(revenue_saved), 0)::text AS revenue_saved,
         ...
         FROM attributed`, [organizationId, branchId]),
        pool.query(`WITH active_members AS (
         SELECT DISTINCT member_id
           FROM subscriptions
          WHERE organization_id = $1
            AND branch_id = $2
       ...
       SELECT COUNT(*)::int AS count
        ...
        WHERE a.last_visit IS NULL OR a.last_visit < $4`, [organizationId, branchId, Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000) - 14 * 86400]),
    ]);
...
async function schedulePtAutomationsForTenant(organizationId, branchId) {
    const settings = await readTenantSettings(organizationId, branchId);
    const automationEnabled = parseBooleanSetting(settings.whatsapp_automation_enabled, true);
    if (!automationEnabled || isStaffInvitesOnlyMode(settings))
        return;
    ...
    const sessionRows = await pool.query(`SELECT s.id AS session_id,
            s.package_id,
            s.member_id,
      ...
      WHERE s.organization_id = $1
        ...
        AND s.scheduled_start < $4::timestamptz`, [organizationId, branchId, reminderStart.toISOString(), reminderEnd.toISOString()]);
    for (const row of sessionRows.rows) {
        ...
        LIMIT 1`, [organizationId, branchId, row.member_id, row.session_id]);
        if (exists.rows[0])
            continue;
        await pool.query(`INSERT INTO message_queue (
    ...
    const packageRows = await pool.query(`SELECT p.id AS package_id,
            p.member_id,
            m.name AS member_name,
            m.phone,
            p.title,
      ...
      WHERE p.organization_id = $1
        ...
        AND p.valid_until <= $4::timestamptz`, [organizationId, branchId, expiryWindowStart.toISOString(), expiryWindowEnd.toISOString()]);
    for (const row of packageRows.rows) {
        ...
        LIMIT 1`, [organizationId, branchId, row.member_id, row.package_id]);
        if (exists.rows[0])
            continue;
        await pool.query(`INSERT INTO message_queue (
          id, organization_id, branch_id, member_id, type, target_phone, target_name, payload, status, attempts, scheduled_at
...
async function bootstrap() {
    ...
    console.log(`WhatsApp worker started (dryRun=${dryRun}, pollMs=${pollMs}, connCheckMs=${connCheckMs}, authBasePath=${authBasePath}, batch=${workerBatchLimit}, minSendIntervalMs=${minSendIntervalMs}, sendJitterMs=${sendJitterMs})`);
